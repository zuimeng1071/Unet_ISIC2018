...
"""
该代码实现了一个改进版的U-Net模型（DASPP_OrthoProj_UNet），结合了以下特点：
1. 正交投影层：通过正交约束的1x1卷积层（OrthogonalConv1x1）实现类似PCA的主成分提取。
   - 正交约束保证投影方向正交，减少特征冗余。
   - 输入数据中心化进一步增强了正交投影的效果。
2. 密集空洞空间金字塔池化（DenseASPP）：通过多尺度空洞卷积提取丰富的上下文信息。
...
class DASPP_OrthoProj_UNet(nn.Module):
    """
...
def main():
    ...
    print(device)
    # 创建模型
    net = DASPP_OrthoProj_UNet().to(device)
    # 创建小规模测试数据
